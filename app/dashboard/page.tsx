    ...
    if (likedIdeas.length > 0) {
      try {
        ...
        if (user) {
          ...
          const count = likedIdeas.length;
          toast.success(count === 1 ? t('dashboard.savedCount').replace('{count}', String(count)) : t('dashboard.savedCountPlural').replace('{count}', String(count)));
        }
      } catch (error) {
          ...
          <div className="space-y-6">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-gray-50 text-gray-500">{t('dashboard.or')}</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-uber-xl p-8 shadow-uber-xl max-w-md mx-4">
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="text-center space-y-2">
                  <h3 className="text-xl font-semibold text-black">{t('dashboard.generatingIdeas')}</h3>
          ...
          <div className="space-y-8">
            <div className="text-center space-y-4 py-8">
              <h2 className="text-3xl md:text-4xl font-bold text-black">
                {savedIdeas.length > 0
                  ? (savedIdeas.length === 1
                    ? t('dashboard.savedCount').replace('{count}', String(savedIdeas.length))
                    : t('dashboard.savedCountPlural').replace('{count}', String(savedIdeas.length)))
                  : t('dashboard.noIdeasSaved')}
              </h2>
              <p className="text-lg text-gray-600">
                {savedIdeas.length > 0
                  ? t('dashboard.viewSaved')
                  : t('dashboard.startNewSearch')}
              </p>
            </div>

            {savedIdeas.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-xl font-semibold text-black">{t('saved.title')}</h3>
            ...
            {maybeLaterIdeas.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <h3 className="text-xl font-semibold text-black">{t('dashboard.reviewLater')}</h3>
                  <span className="text-sm text-gray-500">{t('dashboard.reviewLaterNote')}</span>
                </div>
                <div className="grid gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              <Button onClick={handleNewSearch} size="lg">{t('dashboard.newSearch')}</Button>
              {savedIdeas.length > 0 && (
                <Button variant="outline" size="lg" onClick={() => router.push('/saved')}>
                  {t('dashboard.viewAllSaved')}
                </Button>
              )}
            </div>
          </div>
